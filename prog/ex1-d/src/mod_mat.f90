...
if (mod(N,4)/=0) then
    print*, "[bode_array_int]: array não-múltiplo de 4 detectado."
    stop
end if
...
!**********************************************************************************************************************************!
function differentiate_arr(arr,dx) result(derivative)
!! Função que calcula a derivada de um array `arr` com espaçamento dx. Retorna a derivada `derivative`.
...
if (N<10) then
    print*, "Esta função não está adapatada para tão poucos pontos de `arr`."
    stop
end if
...
!++ Execução
! Para os meus propósitos, esta função deve ser muito precisa. Portanto, usarei diferenças finitas de quarta ordem.
! Pontos onde a diferença finita é de ordem reduzida:
derivative(0) = (arr(1)-arr(0))/dx
derivative(1) = (arr(2)-arr(0))/(2*dx)
derivative(2) = (1/12._dp*arr(0) -2/3._dp*arr(1) + 2/3._dp*arr(3) -1/12._dp*arr(4))/dx
! derivative(3) = (1/12._dp*arr(1) -2/3._dp*arr(2) + 2/3._dp*arr(4) -1/12._dp*arr(5))/dx
! derivative(N-3) = (1/12._dp*arr(N-5) -2/3._dp*arr(N-4) + 2/3._dp*arr(N-2) -1/12._dp*arr(N-1))/dx
derivative(N-2) = (1/12._dp*arr(N-4) -2/3._dp*arr(N-3) + 2/3._dp*arr(N-1) -1/12._dp*arr(N))/dx
derivative(N-1) = (arr(N)-arr(N-2))/(2*dx)
...
do i = 3,N-3
    derivative(i) = -1._dp/60*arr(i-3) + 3._dp/20*arr(i-2) -3._dp/4*arr(i-1) + &
                    3._dp/4*arr(i+1) -3._dp/20*arr(i+2) + 1._dp/60*arr(i+3)
    derivative(i) = derivative(i)/dx
    
end do
! do i = 4,N-4
!     derivative(i) = +1._dp/280*arr(i-4) -4._dp/105*arr(i-3) +1._dp/5*arr(i-2) -4._dp/5*arr(i-1) +&
!                     (-1._dp)/280*arr(i+4) +4._dp/105*arr(i+3) -1._dp/5*arr(i+2) +4._dp/5*arr(i+1)
!     derivative(i) = derivative(i)/dx
